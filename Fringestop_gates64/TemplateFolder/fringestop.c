...
#include <time.h> // for timestamps on output
...
void readbuf2(int fd,char *buf,float *fmax,char *lmap) 
{
	...
	if (read(fd,&head,sizeof(head)) != sizeof(head)) {perror("readbuf:read"); MPI_Abort(MPI_COMM_WORLD,-5);}
	if (read(fd,lmap,NFOLD*FFTLEN/16/NNODE) != NFOLD*FFTLEN/16/NNODE) {perror("readbuf:read"); MPI_Abort(MPI_COMM_WORLD,-5);}
	icount=0;
        for (i=0;i<NFOLD*FFTLEN/16/NNODE;i++) if (lmap[i]>0) imap[icount++]=i;

	if (read(fd,fmax,NCORR*sizeof(float)) != NCORR*sizeof(float)) {perror("readbuf:read"); MPI_Abort(MPI_COMM_WORLD,-5);}
	for (i=0;i<head.lcount;i++)
	if (read(fd,buf+imap[i]*NCORR*2*8,NCORR*2*8) != NCORR*2*8) {perror("readbuf:read"); MPI_Abort(MPI_COMM_WORLD,-5);}
}
