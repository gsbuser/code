...
bool DoOneBuffer(AL4108 *brd)
{
  ...
  if (rank == MASTERNODE || s<1 || s>2) printf("rank %d Buffer Status is %02X\n", rank,s);
  if (s == 3) {
...
//      printf("Skipping Wait because data is already present.\n");
  ...
  if (rank == NNODECORR) printf("rank %d Transfered %li MB in %0.3f seconds, Rate is %0.3f MB/s\n", rank,amountofdata / 1000, sec, mbs);
...
//    int loops = 3600*4; // 1 hour
    int loops = NLOOPS;
    if (rank>=NNODECORR && rank<NNODEACQ+NNODECORR) {
...
//		  	Board->CMS->Set(3);  // 3 -> 5 MHz clock in
    ...
    //if (rank>=NNODECORR+NNODEACQ) break;
    if (rank == 0 && i%4 == 3) if (system("ls -l STOP 2> /dev/null") == 0) break;
    }
  }
  sleep(1); // allow tasks to catch up
