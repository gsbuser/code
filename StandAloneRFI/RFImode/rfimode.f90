use hexprint
integer, parameter :: n=2048,nnode=16,nchan=60,ncorr=nchan*(nchan+1)/2,nred=1!6
...
real xy0(2)
real xy(2,nchan/2)
character*255 fnrfi ! svdrfi input file
character*255 fnpref ! prefix for output files
...
call pmap(trim(trim(adjustl(fnpref))//'.covar.'//trim(adjustl(fn2))//'.pgm'),covar,nchan,nchan,2)
rval=log(max(rval,maxval(abs(rval))/1.e6))
call pmap(trim(trim(adjustl(fnpref))//'.ampl.'//trim(adjustl(fn2))//'.pgm'),rval,n,ncorr,1)
rval=atan2(aimag(rfimode2),real(rfimode2))
call pmap(trim(trim(adjustl(fnpref))//'.phase.'//trim(adjustl(fn2))//'.pgm'),rval,n,ncorr,1)
do i=1,ncorr
   ctmp=0
   ctmp(:n)=rfimode2(:,i)
...
enddo
open(10,file=trim(trim(adjustl(fnpref))//'.lag.'//trim(adjustl(fn2))//'.dat'),form='binary')
write(10) flag
if (localshift) then
   open(10,file=trim(trim(adjustl(fnpref))//'.lagshift.'//trim(adjustl(fn2))//'.dat'))
else
   open(10,file=trim(trim(adjustl(fnpref))//'.lagnoshift.'//trim(adjustl(fn2))//'.dat'))
end if
k=0
do i=1,nchan/2
   do j=i,nchan/2
      ...
      do ipol=1,2
         do jpol=1,2            
            ...
            write(10,*) i,j,ilag,maxval(abs(flag(:,k)))
         end do
      end do
   end do
end do
call pmap(trim(trim(adjustl(fnpref))//'.flag.'//trim(adjustl(fn2))//'.pgm'),flag,2*n,ncorr,2)
end
