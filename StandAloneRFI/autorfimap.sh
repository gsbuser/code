...
echo "make sure you are on gsbm3"

if [[ -z $1 ]]; then
	echo "USAGE: $0 target(rfi or pulsar nume) month(mmm) date(dd) year(yy) startrun lastrun disk"
...
#====================================================================
		...
		#Step323  # SVDRFI
		Step36 #RFImap
	done | tee LOG_${target}_${month}${date}_${run}_map #does not save the error output
done
