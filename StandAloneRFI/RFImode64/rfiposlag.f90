program rfiposlag
integer, parameter :: nx=500,dx=30,nlag=4096,nchan=64,ncorr=nchan*(nchan+1)/2
!integer, parameter :: nx=1000,dx=5,nlag=4096,nchan=64,ncorr=nchan*(nchan+1)/2
real, dimension(-nx:nx,-nx:nx) :: chimap
real, dimension(2,nchan/2) :: xy
...
read(10) flag
flag(nlag/2:nlag/2+2,:)=0
open(20,file='lag.test')
write(20,'(i6)') maxloc(abs(flag),1)-nlag/2
open(20,file='lagpos.test')
k=0
x=-440
y=-680
      do i=1,nchan
...
!            ilag=-delay+nlag/2
            ilag=delay+nlag/2 ! changed 2011/07/05 by upen
...
!chimap=-sqrt(chimap)
call pmap(trim(adjustl(trim(adjustl(fnpref))//'.chimap.'//trim(adjustl(fn2))//'.pgm')),chimap,2*nx+1,2*nx+1,1)
