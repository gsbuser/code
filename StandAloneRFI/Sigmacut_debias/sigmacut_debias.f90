...
real, dimension(n/2,nepoch) :: corr
real, dimension(n) :: corr0
real, dimension(n,ncorr) :: corr1,corrt
real, dimension(n/2,ncorr) :: corr2
integer*1 iline1(n)
integer*1 ilineall(n/2,ncorr,nepoch),ilineall1(n,ncorr),ilineall2(n/2,2,ncorr)
...
do i=1,nepoch
   do j=1,ncorr
      read(10) fmaxall(j,i),ilineall2(:,:,j)
      ilineall(:,j,i)=ilineall2(:,iter,j)
   enddo
   corr2=sign(spread(fmaxall(:,i),1,n/2)*(ilineall(:,:,i)/127.)**2,ilineall(:,:,i)*1.)
   call sigmacut(corr2,n/4,nchan)
   where(corr2 .eq. 0) ilineall(:,:,i)=0
end do
ncut=ncut+count(ilineall .ne. 0)
ncorr1=nchan*(nchan+1)/2-nchan-nchan/2
do j=1,ncorr
   do i=1,nepoch
      corr(:,i)=sign(fmaxall(j,i)*(ilineall(:,j,i)/127.)**2,ilineall(:,j,i)*1.)
   end do
   call rmedian(corr1(1+(iter-1)*n/2,j),corr,nepoch,n/2)
end do
rewind(10)
enddo
write(*,*) 'cut fraction=',1-ncut*1./(n*nepoch*ncorr1)
open(20,file=fnout,form='binary')
ncut=0
do i=1,nepoch
   do j=1,ncorr
      read(10) fmaxall(j,i),ilineall1(:,j)
   enddo
   corrt=sign(spread(fmaxall(:,i),1,n)*(ilineall1/127.)**2,ilineall1*1.)
   call sigmacut(corrt,n/2,nchan)
   where(corrt .eq. 0) ilineall1=0
   ncut=ncut+count(ilineall1 .ne. 0)
   do j=1,ncorr
      corr0=corrt(:,j)!sign(fmaxall(j,i)*(ilineall1(:,j)/127.)**2,ilineall1(:,j)*1.)
      where(corr0 .ne. 0) corr0=corr0-corr1(:,j)
      fmax=maxval(abs(corr0))
      if (fmax .eq. 0) fmax=1
      iline1=nint(127*sign(sqrt(abs(corr0/fmax)),corr0))
      write(20) fmax,iline1
   end do
end do
write(*,*) 'second cut fraction=',1-ncut*1./(n*nepoch*ncorr1)
end
